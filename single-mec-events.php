<?php
/** no direct access **/
defined('MECEXEC') or die();

/**
 * The Template for displaying all single events
 * 
 * @author Webnus <info@webnus.biz>
 * @package MEC/Templates
 * @version 1.0.0
 */
$ticket = get_field('buy_ticket');

get_header('mec'); ?>

    <section id="<?php echo apply_filters('mec_single_page_html_id', 'main-content'); ?>" class="<?php echo apply_filters('mec_single_page_html_class', 'mec-container'); ?>">
 
        <?php do_action('mec_before_main_content'); ?>

        <?php while(have_posts()): the_post(); ?>

            <?php $MEC = MEC::instance(); echo $MEC->single(); ?>

        <?php endwhile; // end of the loop. ?>
        <?php comments_template(); ?>
    </section>

    <?php do_action('mec_after_main_content'); ?>

<?php get_footer('mec');?>
<?php if(!empty($ticket)): ?>
    <script type="text/javascript">
        jq2 = jQuery.noConflict();
jq2(function() {
        jq2(".mec-event-info-desktop").append("<a href='<?php echo $ticket["url"]; ?>' target='<?php echo $ticket['target']; ?>' class='mec-booking-button mec-bg-color my-2' ><?php echo $ticket['title']; ?></a>");
          
});

    </script>
   


<?php endif;